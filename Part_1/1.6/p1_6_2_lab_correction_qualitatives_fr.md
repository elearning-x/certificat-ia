...
Il s'agit des modalités de références (première par ordre alphabétique). La constante (intercept) correspond à une température à midi (`T12`) de 0 degré avec un vent d'Est et un temps nuageux. Quand le temps correspond à cette définition alors on prévoit -20 microgramme par m3 d'ozone. Le modèle dans cette plage n'est pas adapté (car nous n'avons pas de données)
...
La ligne du résumé de `reg` correspondant à `Nord` donne le test de nullité du coefficient correspondant, ce qui correspond ici à la nullité de l'écart de cette modalité `Nord` par rapport à celle de référence `Est` et donc à avoir les deux modalités fusionnées.
...
1.  Représenter graphiquement les données utilisées pour la réponse à la question.
...
La valeur de la statistique de test vaut $26.65$ et sa probabilité critique est quasi nulles, plus petite que $\alpha=1\%$ nous repoussons donc $\mathrm{H}_0$. La provenance a un effet (confirmant le graphique de la question précédente)

La statistique $F$ compare la variabilité entre provenance (numérateur) et la variabilité résiduelle ($\hat\sigma^2$ au dénominateur). Pour améliorer la sensibilité du test il est important d'avoir une petite variabilité résiduelle et donc d'inclure les variables explicatives même si elles ne sont pas dans le questionnement initial, ici la variable `bloc`.
...
Les résidus semblent corrects, le test est très significatif nous sommes donc assez certains de notre conclusion: la provenance a bien un effet sur la hauteur.